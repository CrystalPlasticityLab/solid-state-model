...
namespace tens {
	...
	public:
		...
		friend vector<T, N> vector_product(const vector<T, N>& _lhs, const vector<T, N>& _rhs) {
			...
			const tens::array<T, N>& lhs = static_cast<const tens::array<T, N>&>(_lhs);
			const tens::array<T, N> rhs = _rhs.get_comp_at_basis(_lhs);
	...
	inline void vector<T, N >::normalize() {
		array<T, N>::normalize(*this);//normalize(*this);
	}
	...
	vector<T, N> vector<T, N>::operator - () {
		return  vector<T, N>(tens::array<T, N>::operator-(), *this);
	}
